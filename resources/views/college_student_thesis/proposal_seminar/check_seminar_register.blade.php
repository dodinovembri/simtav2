                ...
                <div class="card card-body pd-sm-40 pd-md-30 pd-xl-y-35 pd-xl-x-40">
                    <div class="tab-content">
                        ...
                        <div id="paneProfile" class="tab-pane active show">
                            <h6 class="tx-uppercase tx-semibold tx-color-01 mg-b-0">Persyaratan Seminar Proposal</h6>
                            <hr>
                            <div class="form-group">
                                <h6>{{ $college_student->nim }} - {{ $college_student->given_name." ".$college_student->middle_name." ".$college_student->surname }}</h6>
                            </div><!-- form-group -->
                            <div class="form-group">
                                <?php foreach ($proposal_seminar_requirements as $key => $value) { ?>
                                    <input type="checkbox" name=""> {{ $value->proposal_seminar_requirement_name }} <br>
                                <?php } ?>
                            </div><!-- form-group -->

                            <div class="form-group">
                                @include('college_student_thesis.components.action')
                            </div><!-- form-group -->
